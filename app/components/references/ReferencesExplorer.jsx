...
export default function ReferencesExplorer({ initialCategories = [] }) {
  ...
  const handleRetry = async () => {
    ...
    setError(null)
    try {
      const data = await fetchCategories({ limit: 100 }, { cache: 'no-store' })
        ...
        {!isLoading && !error && filteredCategories.length > 0 && (
          ...
          >
            {filteredCategories.map((category) => {
              ...
              return (
                <motion.div key={catSlug} variants={cardVariants}>
                  <Link
                  ...
                  >
                    ...
                    <div className='flex items-center justify-between pt-4 border-t border-white/5 text-xs'>
                      <div className='flex items-center gap-1.5 text-gray-400 font-medium'>
                        ...
                        <span>
                          {refCount > 0 ? `${refCount} ${refCount === 1 ? 'reference' : 'references'}` : 'View topics'}
                        </span>
                      </div>
